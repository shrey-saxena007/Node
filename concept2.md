...
`const fs = require('fs');`
- Third-Party Modules: Third-party modules are created by the NodeJS community or external developers and are hosted on package registries like npm (Node Package Manager). Developers can install third-party modules using npm and include them in their applications using require(). Modules are -: express, mongoose, jsonwebtoken.  
